 ...
 */
    if ($(w).width() >= 978) {
      $(w).scrollTop() > 200 ? $('.left-menu>ul').css({
        'position': 'fixed',
        'top': '65px',
 ...
 *  判斷卷軸與第一層選單的連動 
 */
    if ($(w).scrollTop() >= 0 && $(w).scrollTop() < ($('.right-content-1').eq(1).offset().top - 70)) {
      $('.left-menu>ul>li').removeClass('active');
      $('.left-menu-1').eq(0).addClass('active');
      $('.left-menu-1 ul li').removeClass('active');
    }
    if ($(w).scrollTop() >= ($('.right-content-1').eq(1).offset().top - 70) && $(w).scrollTop() < ($('.right-content-1').eq(2).offset().top - 70)) {
      $('.left-menu>ul>li').removeClass('active');
      $('.left-menu-1').eq(1).addClass('active');
      $('.left-menu-1 ul li').removeClass('active');
    }
    if ($(w).scrollTop() >= ($('.right-content-1').eq(2).offset().top - 70) && $(w).scrollTop() < ($('.right-content-1').eq(3).offset().top - 70)) {
      $('.left-menu>ul>li').removeClass('active');
      $('.left-menu ul li.left-menu-1').eq(2).addClass('active');
    }
    if ($(w).scrollTop() >= ($('.right-content-1').eq(3).offset().top - 70)) {
      $('.left-menu>ul>li').removeClass('active');
 ...
 */
  function leftMenuGetSon1(rightSonNum) {
    for (var i = 0; i < rightSonNum - 1; i++) {
      if ($(w).scrollTop() >= ($('.right-content-1-1').eq(i).offset().top - 70) && $(w).scrollTop() < ($('.right-content-1-1').eq(i + 1).offset().top - 70)) {
        $('.left-menu-1 ul li').removeClass('active');
        $('.left-menu-1 ul li').eq(i).addClass('active');
      }
    }
    if ($(w).scrollTop() >= ($('.right-content-1-1').eq(rightSonNum - 1).offset().top - 70) && $(w).scrollTop() < ($('.right-content-1').eq(1).offset().top - 70)) {
      $('.left-menu-1 ul li').removeClass('active');
